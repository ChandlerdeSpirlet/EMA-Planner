        ...
        stripe.balance.retrieve((err, balance) => {
          if (balance) {
            ...
            db.one(failure_query)
              .then(function (row) {
                const checked_in_query = "select count(class_session_id) as week_count from class_signups where class_session_id in (select class_id from classes where starts_at >= (now() - interval '7 hours') - interval '84 hours' and starts_at < (now() - interval '7 hours'));";
